...
class Snapshot():
	def __init__(self, processState, markerId, count,channelOneReceived,channelTwoReceived,channelThreeReceived):		
...
class master_thread(threading.Thread):
	def __init__(self,name,amount,count, inputQueueLock,creditQueueLock,debitQueueLock,snapshotQueueLock,markerCount):
		threading.Thread.__init__(self)
		self.name = name
		self.amount = amount
		self.inputQueueLock = inputQueueLock
		...
		while(True):			
			...
			while(not creditQueue.empty()):							
				...
				if (incomingMessage.getMessageType()=="MESSAGE"):					
					...
					print (self.name).upper() + ": Amount " + str(incomingMessage.getAmountToTransfer()) + " received from " + incomingMessage.getSenderInfo().upper()
					print (self.name).upper() + ": Balance after credit is " + str(self.amount)
					clientName = client2name[incomingMessage.getSenderInfo()]					
					if (bool(snapshotIdValue)):										
						for key,value in snapshotIdValue.items():
							if (clientName == "client1" and value.getChannelOneReceived()==False):					
								value.setChannelOne(incomingMessage.getAmountToTransfer())						
							elif (clientName == "client2" and value.getChannelTwoReceived()==False):					
								value.setChannelTwo(incomingMessage.getAmountToTransfer())						
							elif (clientName == "client3" and value.getChannelThreeReceived()==False):					
								value.setChannelThree(incomingMessage.getAmountToTransfer())						
					...
					if (markerId in snapshotIdValue.keys()):
						...
						if (snapshot.getCount()==3): 
							...
							clientName = client2name[incomingMessage.getSenderInfo()]
							print "Marker" + snapshot.getMarkerId() + " received from " + name_info[clientName].upper()
						...
						print "********************************************************"	
						print (self.name).upper() + ": Received Snapshot Request " + incomingMessage.getSenderInfo().upper()	
						snapshot = Snapshot(self.amount, incomingMessage.getMarkerId(),0,False,False,False)
						clientName = client2name[incomingMessage.getSenderInfo()]
			...
			if (not exitQueue.empty()):
				...
				if (not debitQueue.empty() and isinstance(debitQueue.get(), Message)):
					...
					self.debitQueueLock.release()
					print (self.name).upper() + ": Latest transaction requests sent after quit reverted"
					self.amount += debit.getAmountToTransfer()
				print (self.name).upper() + ": Process exiting. Amount available " +  str(self.amount)
				break			
			...
			if (clientCount==3 and self.count == 400000):				
				...
				self.count = 0
				if (self.amount-debitAmount>0 and debitQueue.qsize() < 5):
					debitRequest = Message("MESSAGE",debitAmount,clientChosen, self.name)
					self.debitQueueLock.acquire()
					debitQueue.put(debitRequest)
					self.debitQueueLock.release()
					self.amount = self.amount-debitAmount
					print (self.name).upper() + ": Sending amount to " + (name_info["client"+str(clientChosen)]).upper() + ", " + str(debitAmount) 	
					print (self.name).upper() + ": Available balance is " + str(self.amount)	
...
class server_thread(threading.Thread):
	...
	def send_info(self):
		...
		while(True):
			...
			if (not exitQueue.empty()):								
				ref_client_info['client1'].send(pickle.dumps(Message("QUIT",0,0,self.name)))
				ref_client_info['client2'].send(pickle.dumps(Message("QUIT",0,0,self.name)))
				ref_client_info['client3'].send(pickle.dumps(Message("QUIT",0,0,self.name)))
				break
			...
			while (not debitQueue.empty() and clientCount==3):
				...
				if (isinstance(debitInfo, Message)):
					...
					ref_client_info[clientInfo].send(pickle.dumps(Message("MESSAGE",debitAmount,0,self.name)))
					##print (self.name).upper() + ": Amount " + str(debitAmount) + " sent to " + str(clientName).upper()
				elif (isinstance(debitInfo, Snapshot)):	
					print "Sending marker to others: " + debitInfo.getMarkerId()									
					ref_client_info['client1'].send(pickle.dumps(Message("MARKER",0,debitInfo.getMarkerId(),self.name)))
					ref_client_info['client2'].send(pickle.dumps(Message("MARKER",0,debitInfo.getMarkerId(),self.name)))
					ref_client_info['client3'].send(pickle.dumps(Message("MARKER",0,debitInfo.getMarkerId(),self.name)))
...
def process(argv):
	...
	master = master_thread(name_info['server'],int(balance),0,inputQueueLock,creditQueueLock,debitQueueLock,snapshotQueueLock,1)
